myPno <- function (path_bioclim, path_model, subset = NULL, bin_width = 1, 
          bin_number = NULL) 
{
  ...
  cat("\nBioclimatic data ranges from", bc_min, "to", bc_max, 
      "and will be binned into", length(cats), "categories with a bin width of", 
      by, "\n")
  ...
  x <- matrix(nrow = length(cats), ncol = length(specs) + 
                1)
  ...
  for (h in seq(along = specs)) {
    ...
    if (px > 0) 
      cat("\n\nWARNING: Raster maps differ by", px, "cells", 
